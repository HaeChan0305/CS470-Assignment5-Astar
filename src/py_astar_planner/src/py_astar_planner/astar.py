...
def astar_planning(start, goal, actions, grid_limits,
    ...
    while True:
        ...
        cur_idx = min(openset.keys(), key = lambda x : openset[x].cost + openset[x].h)
        cur_node = openset[cur_idx]

        if goal_node.idx == cur_node.idx:
        ...
        for i, action in enumerate(actions): 
            ...
            if is_valid(next_pos, grid_limits, costmap, robot_size, threshold) is False:
                continue
            ...
            next_node = Node(next_pos, next_idx, 
                             cur_node.cost + 1,
                             np.sum(np.abs(np.array(next_pos)-goal), axis=0), cur_idx)
            try:
                if openset[next_idx].cost + openset[next_idx].h > next_node.cost + next_node.h:
    ...
    while prev_idx != -1:
        cur_node = CLOSED_SET[prev_idx]
        path.append(cur_node.pos.tolist())
        prev_idx = cur_node.prev_idx

    #assert path[-1] == start, f"It can't be reachable :Last pos({path[-1]}) of path MUST be start pos({start})"
    #print("num closed_set", len(CLOSED_SET))
    #------------------------------------------------------------
